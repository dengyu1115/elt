...
import com.nature.kline.common.model.Net;
...
public class NetMapper {
    ...
    public int batchMerge(List<Net> list) {
        SqlParam param = SqlParam.build().append("replace into net(code, date, net, rate, net_total, rate_total)")
    ...
    public List<Net> listByCode(String code) {
        SqlParam param = SqlParam.build().append("select code, date, net, rate, net_total, rate_total " +
                "from net where code = ?", code);
        return baseDB.list(param, mapper);
    }

    public List<Net> listAfter(String code, String date) {
        SqlParam param = SqlParam.build().append("select code, date, net, rate, net_total, rate_total from net")
    ...
    public void delete() {
        baseDB.executeUpdate(SqlParam.build().append("delete from net"));
    }
