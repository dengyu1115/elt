...
public class ComponentStarter {
    ...
    public static ComponentStarter getInstance() {
        if (instance == null) synchronized (ComponentStarter.class) {
            if (instance == null) instance = new ComponentStarter();
        }
        return instance;
    }

    public void start() {
        for (Class<?> cls : InjectClasses.CLASSES) this.inject(cls, InstanceHolder.get(cls));
    }

    private void inject(Class<?> cls, Object o) {
        Method[] methods = cls.getMethods();
        for (Method m : methods)
            if (m.isAnnotationPresent(TaskMethod.class))
                TaskHolder.put(m.getAnnotation(TaskMethod.class).value(), m, o);
        ...
        if (!sc.equals(Object.class)) this.inject(sc, o);
    }
}
